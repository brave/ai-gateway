...
def s3_put_fails(ctx, monkeypatch):
    monkeypatch.setattr(
        share_api,
        "_S3_SESSION",
        _mock_s3_session(put_object=AsyncMock(side_effect=RuntimeError("s3 down"))),
    )
...
def s3_get_no_such_key(ctx, monkeypatch):
    monkeypatch.setattr(
        share_api,
        "_S3_SESSION",
        _mock_s3_session(get_object=AsyncMock(side_effect=_client_error("NoSuchKey"))),
    )
...
def s3_get_unexpected(ctx, monkeypatch):
    monkeypatch.setattr(
        share_api,
        "_S3_SESSION",
        _mock_s3_session(get_object=AsyncMock(side_effect=RuntimeError("boom"))),
    )
...
def sidecar_get_denied(ctx, monkeypatch):
    monkeypatch.setattr(
        ...
        "_S3_SESSION",
        _mock_s3_session(
            get_object=AsyncMock(side_effect=_client_error("AccessDenied"))
        ),
    )
...
def sidecar_objects_assert(ctx, caplog):
...
def delete_success_assert(ctx):
    ...
    puts = [call for call in ctx["s3_calls"] if call[0] == "put"]
    assert puts == []
    deletes = [call for call in ctx["s3_calls"] if call[0] == "delete"]
