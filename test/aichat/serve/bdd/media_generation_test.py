...
def _(ctx, payload):
    # Malformed variants live in the dedicated malformed-payload given.
    ctx["triton_data"] = {
        "named_output_str": {
...
def _(ctx):
    ...
    assert kwargs == {"model": "tts-m", "voice": ctx["voice"]}
...
def _(ctx, monkeypatch):
    ...
    # Create and drain inside ONE loop so the httpx-backed generator never
    # crosses event loops.
    async def _run():
        stream = await tts_gen.stream_speech("tts-m", {"input": "hello"})
        return [c async for c in stream]

    ctx["chunks"] = asyncio.run(_run())
...
def _(ctx, url, auth):
    ...
    # api_key "none" -> prod default "empty" (tts/generator.py default), so
    # the header becomes "Bearer empty" by design, not by accident.
...
@then("the upstream speech client is closed")
def _(ctx):
    # stream_speech must close its httpx client when the body is drained.
...
def _(ctx, monkeypatch):
    ...
    ctx["client"] = _fake_tts_http(monkeypatch, upstream)
...
def _(ctx, text):
    ...
    # The 500 path must release both the upstream response and the client.
    assert ctx["upstream"].aclose.await_count == 1
    assert ctx["client"].aclose.await_count == 1
...
def _(ctx, count, rows):
    ...
    # data is nested once on purpose: prod sends data=[inputs_list], so a
    # 2-string request carries one row of two items with shape [2, 1].
    assert inp["data"] == ast.literal_eval(rows)


@given(
    parsers.parse(
        "a triton embeddings response with {input_count:d} inputs and an 8-float data array"
    )
)
...
def _(ctx):
    ...
    assert kwargs == {"model": "img-m", "prompt": ctx["prompt"]}
...
@when("passthrough kwargs are built")
def _(ctx, monkeypatch):
    monkeypatch.setattr(
...
@when(
    parsers.re(
        r"the backend payload (?P<payload>.+) is formatted with route_only "
        r'"(?P<route_only>true|false)" and include_routing '
        r'"(?P<include_routing>true|false)"'
    )
)
...
@when(
    parsers.re(
        r"the backend payload (?P<payload>.+) is formatted with route_only "
        r'"(?P<route_only>true|false)"$'
    )
)
def _(ctx, payload, route_only):
    # Distinguish the two failure modes: one payload each.
    payload_desc = {
        "none_routing": "{'answers': ['a']}",
        "none_answers": "{'routing': 'router'}",
    }[payload]
    try:
...
@when(parsers.parse('run_system_one executes for model "{model}" with a state payload'))
