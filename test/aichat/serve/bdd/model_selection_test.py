...
def _install_models(monkeypatch, models: dict):
    ...
    mock.models = models
    mock.model_triaging = {}
    mock.content_agent_default_model = "agent-model"
...
def _clear_model_config_cache(monkeypatch):
    get_model_config.cache_clear()
    yield
    get_model_config.cache_clear()


def _model_entry(**overrides):
    base = {"type": "llm", "capabilities": [], "free": True}
...
@given(
    parsers.parse(
        'triaging {category} premium "{premium}" and non-premium "{non_premium}"'
    )
)
...
def _install_settings(monkeypatch, ctx, **overrides):
    mock = MagicMock()
    mock.models = ctx.get("models", {})
    mock.model_triaging = ctx.get("triaging", {})
    mock.content_agent_default_model = overrides.get(
        "content_agent_default_model",
        ctx.get("content_agent_default_model", "agent-model"),
    )
    monkeypatch.setattr(model_selection, "model_settings", mock)
...
def given_random_favors(position, monkeypatch):
    picker = MagicMock()
    picker.choices = MagicMock(return_value=["a-model"])
    monkeypatch.setattr(model_selection, "random", picker)
...
def given_androcles_classifies(task, monkeypatch):
    probabilities = [0.1] * 11
    index = {"vision": 8, "language": 10, "coding": 3}[task]
    probabilities[index] = 0.95
    monkeypatch.setattr(
        model_selection, "androcles_inference", AsyncMock(return_value=probabilities)
    )


@given("androcles classification returns nothing")
def given_androcles_none(monkeypatch):
    monkeypatch.setattr(
        model_selection, "androcles_inference", AsyncMock(return_value=None)
    )
...
@when(parsers.parse('a model is selected for "{model}"'))
