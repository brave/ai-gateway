...
@given(parsers.parse('a model "cap-model" with type "{model_type}"'))
def given_cap_model_type(model_type, monkeypatch, ctx):
    given_model_type("cap-model", model_type, monkeypatch, ctx)
...
@given(parsers.parse('a model "{model_id}" with backend "{backend}"'))
...
@given(parsers.parse('fallback model "{fallback}" with backend "{backend}"'))
...
@given(parsers.parse('a model "{model_id}" with upstream model "{upstream}"'))
...
@given(
    parsers.parse(
        'a model "{model_id}" with upstream model "{upstream}" on backend "{backend}"'
    )
)
...
def given_shared_profiles(model_ids, env_name, monkeypatch, ctx):
    ...
    ctx["model_id"] = model_ids.split(",")[0]
    _install(monkeypatch, ctx)
...
def then_warning_logged(caplog, ctx):
    assert len(caplog.records) > ctx["warning_count_before"]
...
@then(parsers.parse('the first model display language is "{language}"'))
def then_display_language(language, ctx):
    assert models_api._parse_accept_language(ctx["accept_language"]) == language
...
def then_options_name(model_id, ctx):
    entry = ctx["response"].json()[0]
    assert entry["options"]["name"] == model_id


@then(parsers.parse('the catalog entry access is "{access}"'))
def then_access(access, ctx):
    entry = ctx["response"].json()[0]
