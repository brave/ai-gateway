...
def _(ctx, field):
    metadata = ctx["auth_calls"][0]["metadata"]
    assert field not in metadata
    assert metadata["model"] == "llama-2-13b-chat"
...
def _(ctx):
    ctx["other"] = create_idempotency_key(ctx["messages"], "mixtral-8x7b-instruct")
...
def _(monkeypatch):
    ...
    monkeypatch.setattr(internal_client, "sku_verify", _down)
