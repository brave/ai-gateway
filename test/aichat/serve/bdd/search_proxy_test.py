...
def compute_cors(ctx, origin, monkeypatch):
    monkeypatch.setattr(search_settings, "brave_search_api_url", "https://bsa.ai")
...
def upstream_200(ctx, payload, monkeypatch):
    client = MagicMock()
...
def upstream_non_200(ctx, status_code, monkeypatch):
    client = MagicMock()
...
def upstream_transport_drops(ctx, monkeypatch):
    client = MagicMock()
...
def rich_fetch_failure_assert(ctx):
