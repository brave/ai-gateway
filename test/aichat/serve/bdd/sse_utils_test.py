...
def given_catalog(model_id, upstream, ctx):
    ctx["catalog"] = {
...
def given_bedrock_profile(ctx):
    # The catalog entry for "titan" (with its inference profile env) is
    # installed by the shared catalog given; this step only pins the name.
    ctx["bedrock_profile_model"] = "titan"
...
def when_normalize(litellm, monkeypatch, ctx):
    monkeypatch.setattr(model_settings, "models", ctx["catalog"])
    try:
...
@then("no chunk is longer than 5 characters")
...
def given_counting_input(description, ctx):
    kind = description.replace(" ", "_").replace("-", "_")
    ctx["counting_kind"] = kind
    if kind == "a_broken_tokenizer" or kind.startswith("calculate"):
        ctx["counting_messages"] = [{"role": "user", "content": "hello there friend"}]


@when("the tokens are counted")
def when_count_tokens(ctx):
...
def given_libc_lookup(libc_lookup, ctx):
...
def when_load_malloc_trim(ctx, monkeypatch):
    ...
    if lookup == "missing":
        ...
        monkeypatch.setattr(
            ctypes, "CDLL", MagicMock(return_value=SimpleNamespace(malloc_trim=_trim))
        )
    else:  # no trim symbol
        monkeypatch.setattr(ctypes.util, "find_library", lambda name: "libc")
...
def given_counting_trim(ctx, monkeypatch):
    ctx["trim_calls"] = []

    def _fake_trim(pages):
        ctx["trim_calls"].append(pages)

    monkeypatch.setattr(utils, "_malloc_trim", _fake_trim)


@given("no trim function is available")
def given_missing_trim(monkeypatch):
    monkeypatch.setattr(utils, "_malloc_trim", None)


@when("the periodic trim loop runs for one tick")
def when_periodic_trim(ctx):
    ctx.setdefault("trim_calls", [])

    async def run():
        task = asyncio.create_task(utils.periodic_malloc_trim(0))
        # Deterministic: wait until the first trim tick actually happened
        # instead of relying on a wall-clock sleep being long enough on CI.
        if utils._malloc_trim is not None:
            for _ in range(200):
                if ctx["trim_calls"] or task.done():
                    break
                await asyncio.sleep(0.005)
        else:
            # No trim fn: the loop never records a call; a short soak is enough.
...
def then_trim_loop_trims(ctx):
    assert ctx["trim_loop"] == "cancelled"
    assert ctx["trim_calls"], "loop must call the trim function each tick"
    assert all(pages == 0 for pages in ctx["trim_calls"])


@then("the loop exits cleanly without trimming")
def then_trim_loop(ctx):
    assert ctx["trim_loop"] == "cancelled"
    assert not ctx.get("trim_calls")
