...
def given_completion_response(description, ctx):
    ...
    elif description == "a plain dict":
        response = {
            "choices": [{"message": {"content": "partial"}, "finish_reason": "stop"}]
        }
    else:
        raise AssertionError(f"unknown completion response: {description!r}")
...
def given_stream_state(text, finish_reason, tool_calls, ctx):
    # "<empty>" is the outline sentinel for an empty assistant text cell
    # (Gherkin parse patterns cannot match an empty string).
    ctx["assistant_text"] = "" if text == "<empty>" else text
...
def given_record(action, model, ctx):
    ...
    def _sample(action_label):
        return (
            REGISTRY.get_sample_value(
                "truncation_recovery_total", {"model": model, "action": action_label}
            )
            or 0
        )
    ...
    ctx["record_metric"] = (
        "skipped_degenerate_stream" if action == "retry" else "continue"
    )
    ctx["record_before"] = _sample(ctx["record_metric"])


@when("the recovery is recorded")
def when_recovery_recorded(ctx):
    tc.record_stream_truncation_recovery(
        model=ctx["record_model"], action=ctx["record_action"], depth=1
    )
...
def then_skip_metric(ctx):
    ...
    after = (
        REGISTRY.get_sample_value(
            "truncation_recovery_total",
            {"model": ctx["record_model"], "action": ctx["record_metric"]},
        )
        or 0
    )
    assert ctx["record_action"] == "retry"
    assert ctx["record_metric"] == "skipped_degenerate_stream"
    assert after == ctx["record_before"] + 1
...
def then_continue_metric(ctx):
    ...
    assert ctx["record_metric"] == "continue"
    assert after == ctx["record_before"] + 1
...
def given_near_stream_request(ctx, monkeypatch):
