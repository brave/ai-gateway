Feature: Media sandbox primitives
  ...
  Scenario Outline: Protocol request encoding rejects oversized payloads
    ...
    Then encoding raises ProtocolError <expected>

    Examples:
      | defect                     | expected                  |
      | a huge string argument     | for the single argument   |
      | a huge serialized payload  | for the whole payload     |
  ...
  Scenario Outline: The worker serve loop handles each input line
    ...
    Examples:
      ...
      | is blank then a valid ping        | skips the blank line and answers the next request |
  ...
  Scenario Outline: PDF analysis handles page counts and token budgets
    ...
    Then the pdf analysis outcome is "<outcome>"
