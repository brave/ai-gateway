Feature: Media service generators
  ...
  Scenario Outline: Speech streaming proxies the upstream model address
    ...
    And the upstream speech client is closed
  ...
  Scenario Outline: Triton embedding responses transform back to OpenAI format
    Given a triton embeddings response with <input_count> inputs and an 8-float data array
  ...
  Scenario Outline: SystemOne response formatting respects route_only and routing flags
    ...
    Examples:
      ...
      | true       | false           | {'model': 'so-external', 'routing': 'router'}                    |

  Scenario Outline: SystemOne response formatting rejects missing payloads
  ...
  Scenario: SystemOne run uses the passthrough kwargs and decodes the response
    When run_system_one executes for model "so-model" with a state payload
