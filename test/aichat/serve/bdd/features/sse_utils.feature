Feature: Serve utility helpers
  ...
  Scenario: Static content streams as five character chunks
    ...
    And the last chunk is not longer than 5 characters
  ...
  Scenario Outline: Token counting handles messages, parts and failures
    ...
    Then the token count is greater than zero <validity>

    Examples:
      | description                      | validity |
      | plain text messages              | valid    |
      | multimodal text parts            | valid    |
      | a broken tokenizer               | fallback |
      | calculate plain content          | valid    |
      | calculate non-text part          | valid    |
      | calculate tool calls             | valid    |
