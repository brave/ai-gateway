Feature: NEAR TEE attestation
  ...
  Scenario Outline: Attestations are verified against the pre-verified registries
    ...
    And the s3 heads return <s3_state>
    When the attestation is verified
    Then verification is <verdict>
    And the s3 heads checked <headed_keys>

    Examples:
      | sig_state | keys_state   | compose_state | s3_state | verdict    | headed_keys                                                  |
      | valid     | registered   | verified      | objects  | successful | pre-verified/abc123.json + signing-keys/{signing_key}.json           |
      | valid     | registered   | missing       | objects  | failed     | none                                                         |
      | tampered  | registered   | verified      | objects  | failed     | none                                                         |
      | valid     | unregistered | verified      | objects  | failed     | none                                                         |
      | valid     | registered   | mismatched    | objects  | failed     | none                                                         |
      | valid     | registered   | verified      | missing  | failed     | pre-verified/abc123.json                                     |
  ...
  Scenario: An attestation without a gateway app compose is verified, built and cached
    Given a fresh attestation for "near-model" that verifies cleanly
    And a model "near-model" addressed at "https://addr/v1/"
    When the ohttp config is verified and fetched for "near-model"
    Then the built config is returned and stored in redis

  Scenario: A fully digested gateway compose is verified, built and cached
    Given a fresh attestation for "near-model" with a fully digested compose
    And a model "near-model" addressed at "https://addr/v1/"
    When the ohttp config is verified and fetched for "near-model"
    Then the built config is returned and stored in redis
    And the target digests were pre-verified in s3
