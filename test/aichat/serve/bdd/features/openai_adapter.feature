Feature: OpenAI adapter security and stream helpers
  ...
  Scenario Outline: Chunk emitters inject compaction metadata once
    ...
    And a second emission carries no metadata

    Examples:
      | with_metadata | first_chunk        | metadata_expected |
      | yes           | data: {"id":1}     | yes               |
      | no            | data: {"id":1}     | no                |
      | yes           | not-json-chunk     | no                |
  ...
  Scenario Outline: Long responses are truncated for the trace
    ...
    Then the truncated response starts with "..." <ellipsis> and keeps the tail
