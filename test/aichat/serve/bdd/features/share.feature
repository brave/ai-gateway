Feature: Share storage API
  ...
  Scenario: Share creation reports a storage failure and leaves an orphaned sidecar
    Given the s3 put fails after the sidecar succeeds
    When a share is created with a valid ciphertext
    Then the response is an internal server error
    And the deletion sidecar was written but the share was not
  ...
  Scenario: Share deletion rejects invalid services keys
    When the deletion "deletion-1" is posted with an invalid services key
    Then the response is an invalid auth key error
