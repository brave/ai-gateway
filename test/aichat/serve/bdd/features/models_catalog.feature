Feature: Model catalog and configuration
  ...
  Scenario: Empty litellm names normalize to nothing
    Given a model "friendly-1" with upstream model "org/friendly-upstream"
    When the litellm name "none" is normalized
    Then the normalized model is "none"
  ...
  Scenario: Non-bedrock models carry no inference profile
    ...
    When the litellm name "vllm-1" is normalized with requested model "vllm-1"
    Then the normalized model is "vllm-1"
  ...
  Scenario Outline: Accept-Language parsing for model listing
    ...
    Then the first model display language is "<language>"

    Examples:
      | accept_language         | language |
      | de-DE,de;q=0.9,en;q=0.8 | de       |
      | not-a-number;q=abc,fr   | not      |
      | nothing                 | en       |
  ...
  Scenario: The models endpoint returns catalog entries
    ...
    And the catalog entry access is "basic_and_premium"