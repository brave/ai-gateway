...
COUNT_WORDS = {"zero": 0, "one": 1, "two": 2, "three": 3, "four": 4}
...
@then(parsers.parse("{count} sources are produced"))
def then_count(ctx, count):
    assert len(ctx["formatted"]["sources"]) == COUNT_WORDS[count]
...
@then("no rich results are attached")
def then_no_rich(ctx):
    assert "rich_results" not in ctx["formatted"]
