...
def given_transport_failure(ctx, monkeypatch, failure):
    key = failure.replace(" ", "_")
    exc = {
        "timeout": httpx.TimeoutException("t"),
        "connect_error": httpx.ConnectError("c"),
        "generic_request": httpx.RequestError("r"),
    }[key]
    # Executor maps TimeoutException to "timed out" and every other
    # transport failure to "unreachable"; pin the exact substring.
    ctx["expected_error"] = "timed out" if key == "timeout" else "unreachable"
    monkeypatch.setattr(
...
def then_transport_error(ctx):
    ...
    assert ctx["expected_error"] in first["error"]
...
def then_web_sources_first(ctx):
    ...
    ws_idx = next(
        (i for i, s in enumerate(strings) if "brave-chat.webSources" in s), None
    )
    assert ws_idx is not None, strings
    # Match the completion by its answer text, not a generic "---" marker any
    # markdown chunk could carry.
    completion_idx = next((i for i, s in enumerate(strings) if "The answer" in s), None)
    assert completion_idx is not None, strings
    assert completion_idx > ws_idx
    ctx["completion"] = strings[completion_idx]
...
def then_tool_message_answer(ctx):
    ...
    # The handler path packages the answer as list content.
    assert isinstance(message.content, list), type(message.content)
...
def then_completion_yielded(ctx):
    # Match by answer content, not position: prod may emit other events first.
    completion = next(
        (e for e, _ in ctx["events"] if e and "The answer [1]." in e), None
    )
    assert completion is not None, ctx["events"]
    assert completion.startswith("data: ")
    payload = json.loads(completion.removeprefix("data: "))
    # format_answer_with_citations prefixes the report with the separator.
