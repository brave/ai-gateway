...
#
# TODO: remove test/aichat/serve/ohttp_test.py#TestGetModelOhttpConfig#test_returns_config_for_valid_model test/aichat/serve/ohttp_test.py:65
# TODO: remove test/aichat/serve/ohttp_test.py#TestGetModelOhttpConfig#test_returns_404_for_unknown_model test/aichat/serve/ohttp_test.py:82
# TODO: remove test/aichat/serve/ohttp_test.py#TestGetModelOhttpConfig#test_returns_404_for_model_without_e2ee test/aichat/serve/ohttp_test.py:91
# TODO: remove test/aichat/serve/ohttp_test.py#TestGetModelOhttpConfig#test_returns_500_when_near_verify_fails test/aichat/serve/ohttp_test.py:100
# TODO: remove test/aichat/serve/ohttp_test.py#TestGetModelOhttpConfig#test_returns_401_for_invalid_auth test/aichat/serve/ohttp_test.py:116
# TODO: remove test/aichat/serve/ohttp_test.py#TestRelayModelOhttp#test_proxies_request_and_streams_response test/aichat/serve/ohttp_test.py:142
...
# TODO: remove test/aichat/serve/ohttp_test.py#TestRelayModelOhttp#test_returns_500_when_near_verify_fails test/aichat/serve/ohttp_test.py:214
...
from aichat.serve.services.near import OHTTPConfig
...
_OHTTP_CONFIG = {
    ...
    "upstream_model_name": "near-upstream",
}
...
def model_with_e2ee(ctx, model_name, monkeypatch):
    ms = SimpleNamespace(models={model_name: dict(_E2EE_MODEL)})
...
def model_without_e2ee(ctx, model_name, monkeypatch):
    ms = SimpleNamespace(models={model_name: {"address": "https://example.ai/v1/"}})
    monkeypatch.setattr(ohttp_mod, "model_settings", ms)


@given("no models are configured")
def no_models(ctx, monkeypatch):
    ms = SimpleNamespace(models={})
    monkeypatch.setattr(ohttp_mod, "model_settings", ms)


@given("near verification returns the ohttp config")
def near_verify_ok(ctx, monkeypatch):
    # Prod returns an OHTTPConfig model, not a dict — use the real class so
    # any attribute access in the relay path is exercised, not masked.
    verify = AsyncMock(return_value=OHTTPConfig.model_validate(_OHTTP_CONFIG))
...
def upstream_returns(ctx, content, monkeypatch):
    ...
    monkeypatch.setattr(ohttp_mod.httpx, "AsyncClient", lambda *a, **k: client)
    ctx["client"] = client
...
def common_params_fail(ctx, monkeypatch):
    ...
    # Guard: the relay must return the common-params error before any
    # upstream contact — constructing an AsyncClient raises loudly.
    def _no_upstream(*a, **k):
        raise AssertionError("upstream must not be contacted when common params fail")

    monkeypatch.setattr(ohttp_mod.httpx, "AsyncClient", _no_upstream)
...
def ohttp_config_assert(ctx):
    resp = ctx["response"]
    # Endpoint returns the config as a model; compare its full payload.
    assert resp.model_dump() == _OHTTP_CONFIG
...
def relay_stream_assert(ctx, content, media_type):
    ...
    # stream_and_close must release the upstream body and the client.
    assert ctx["client"].aclose.await_count == 1
...
def upstream_request_assert(ctx, gateway_url):
    ...
    assert call.args[1] == gateway_url
    # Required headers as a subset: prod may add more (e.g. user-agent)
    # without that being a spec change.
    headers = call.kwargs["headers"]
    assert headers["Authorization"] == "Bearer key"
    assert headers["content-type"] == "message/ohttp-req"
    assert call.kwargs["content"] == b"ohttp-request-bytes"
