...
def near_harness(ctx, monkeypatch):
    ...
    monkeypatch.setattr(
        near_mod.aioredis, "Redis", lambda connection_pool=None: ctx["redis"]
    )
...
def build_config(ctx):
    ...
    ctx["built"] = _build_ohttp_config(attestation, "near-model")
...
def _compose_app_compose(images):
    ...
    compose = {
        "services": {
            name: {"image": image} for name, image in zip(services, images, strict=True)
        }
    }
    return json_mod.dumps(
        {"docker_compose_file": json_mod.dumps({"services": compose["services"]})}
    )
...
def _attestation_for(ctx, *, tampered=False, registered=True, compose=True):
...
def verify_attestation(ctx):
    attestation = _attestation_for(
        ...
        compose=ctx["att_compose_state"] == "verified",
    )
    ctx["verdict"] = _verify_attestation(attestation, "near-model")
...
def redis_cached(ctx):
    ...
    key = near_mod.NEAR_ATTEST_RESULT_CACHE_KEY.format(model_name="near-model")
