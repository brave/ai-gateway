...
def when_resolve_model(ctx, monkeypatch):
    mp = _new_patch(ctx)
    mp.setattr(
...
def when_dleo_disabled(ctx, monkeypatch):
    mp = _new_patch(ctx)
...
def when_dleo_no_text(ctx, monkeypatch):
    mp = _new_patch(ctx)
...
def given_dleo_categories(ctx, monkeypatch):
    mp = _new_patch(ctx)
    raw_config = {
        "tracking": {
...
def when_prefetch_raises(ctx, monkeypatch):
    mp = _new_patch(ctx)
    mp.setattr(model_settings, "models", {"embed-x": {"type": "embedding"}})
...
def then_keyword_only(ctx):
    assert "tracking" in ctx["prefetch"].matched_categories
    assert ctx["prefetch"].task_type is None or isinstance(
        ctx["prefetch"].task_type, str
    )
...
def given_keywords_both_turns(ctx, monkeypatch):
    mp = _new_patch(ctx)
    mp.setattr(dynamic_leo_settings, "enable_dynamic_leo", True)
    mp.setattr(
...
def when_dleo_keywords(ctx, monkeypatch):
    mp = _new_patch(ctx)
    mp.setattr(
        signals,
        "androcles_inference",
