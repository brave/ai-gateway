...
from test.aichat.serve.bdd.helpers import (
    completion_response as _completion,
)
from test.aichat.serve.bdd.helpers import (
...
def when_run_nonstream(ctx):
    ctx["request"].stream = False
    when_run(ctx)


@when("the conversation title completion runs with streaming")
def when_run_stream(ctx):
    ctx["request"].stream = True
    ctx["stream_received"] = []

    def fake_stream(*args, **kwargs):
        ctx["stream_received"].append(args[0] if args else kwargs.get("response"))
...
def then_streamed(ctx):
    ...
    # The backend reply must be handed straight to the streaming stack —
    # the title path proxies whatever converse() produced.
    assert ctx["stream_received"][0] is ctx["backend"].reply
