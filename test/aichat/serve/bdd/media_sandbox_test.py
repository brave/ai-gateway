...
def request_encode_defect(ctx, defect, monkeypatch):
    # Keep allocations tiny: the size limits are module constants, so lower
    # MAX_MESSAGE_BYTES instead of building 64 MB strings.
    monkeypatch.setattr(protocol, "MAX_MESSAGE_BYTES", 10)
    if defect == "a huge string argument":
        req = protocol.Request(id=1, op="op", args={"k": "x" * 11})
    else:
        req = protocol.Request(id=1, op="op", args={"k": "xxxxx", "j": "yyyyy"})
    try:
        req.encode()
        ctx["error"] = None
...
@then(parsers.parse("encoding raises ProtocolError {expected}"))
def encoding_raises(ctx, expected):
    assert isinstance(ctx["error"], protocol.ProtocolError)
    msg = str(ctx["error"])
    if expected == "for the single argument":
        assert msg.startswith("arg 'k' too large"), msg
    elif expected == "for the whole payload":
        assert msg.startswith("request too large"), msg
    else:
        raise AssertionError(f"unknown encode expectation: {expected!r}")
...
def limit_application_outcome(ctx, outcome, caplog):
    if outcome == "sets the new soft and hard limits":
        ...
        # Unenforceable limits degrade to a warning, not a crash.
        assert ctx["recorded"] == []
        assert any(
            "not enforceable on this platform" in r.message for r in caplog.records
        )
...
def scrub_env(ctx, monkeypatch):
    # scrub_environment() strips os.environ in-process; snapshot and restore
    # every key so later tests keep their env.
    saved = dict(os.environ)
    try:
        os.environ.setdefault("PATH", "/usr/bin:/bin")
        monkeypatch.setenv("SANDBOX_EVIL_VAR", "1")
...
def serve_outcome(ctx, outcome):
    ...
    elif outcome == "skips the blank line and answers the next request":
        assert json.loads(ctx["out"]) == {"id": 5, "ok": True, "value": {"pong": True}}
    elif outcome == "exits the loop":
        assert ctx["out"] == ""
    else:
...
def init_sandbox_non_linux(ctx, monkeypatch):
    ...
    # Pin TMPDIR through monkeypatch so the scratch dir init_sandbox
    # installs is restored automatically after the scenario. Also register
    # tempfile.tempdir with monkeypatch so init_sandbox's override of the
    # module global is undone (the scratch dir itself is removed below).
    monkeypatch.setenv("TMPDIR", os.environ.get("TMPDIR") or tempfile.gettempdir())
    # Register tempfile.tempdir with monkeypatch so init_sandbox's override
    # of the module global is undone.
    monkeypatch.setattr(tempfile, "tempdir", tempfile.tempdir, raising=False)
    # Register TIKTOKEN_CACHE_DIR so init_sandbox's setdefault cannot leak an
    # unregistered key into the process environment. NOTE: pre-setting ""
    # means prod's setdefault never fires here — accepted divergence, this
    # scenario pins the tempdir/TMPDIR handling instead.
...
def decode_wav(ctx, wav_desc, monkeypatch):
    ...
    elif wav_desc == "is malformed wave data":
        payload = b"RIFFnotawavefile"
    else:
        raise AssertionError(f"unknown wav payload in feature: {wav_desc!r}")
...
def make_pdf(ctx, pdf_desc):
    ...
    if pdf_desc == "with 2 blank pages":
        ...
        for _ in range(90):
            writer.add_blank_page(width=612, height=792)
    elif pdf_desc == "needing native truncation":
        for _ in range(3):
            writer.add_blank_page(width=612, height=792)
    else:
        raise AssertionError(f"unknown pdf description in feature: {pdf_desc!r}")
    buf = io.BytesIO()
    writer.write(buf)
...
def analyze_sandbox_pdf(ctx, budget, monkeypatch):
    ...
    if "native truncation" in ctx.get("pdf_desc", ""):
        monkeypatch.setattr(
            ops_pdf,
...
@then(parsers.parse('the pdf analysis outcome is "{outcome}"'))
def analysis_outcome(ctx, outcome):
    ...
    if outcome == "reports 2 pages under the budget":
        ...
        assert "page 3 content" in res["overflow_text"]
    else:
...
def run_probe(ctx, op, monkeypatch):
    if op == "probe_env":
        ...
        # guard: landlock/ops read platform.system() at runtime
        assert platform.system() == "Darwin"
        ctx["probe"] = ops_probe.op_probe_fs_write()
    elif op == "spin_cpu":
        ctx["probe"] = ops_probe.op_spin_cpu(seconds=0)
    else:
        raise AssertionError(f"unknown probe op in feature: {op!r}")
...
def probe_outcome(ctx, outcome):
    ...
    elif outcome == "spins for the requested duration":
        # op_spin_cpu reports the REQUESTED duration (min(seconds, 60)), not a
        # measured elapsed time — exact 0.0 is deterministic.
        assert res == {"spun": 0.0, "x": 0}
    else:
        raise AssertionError(f"unknown probe outcome: {outcome!r}")
...
def landlock_unavailable(ctx):
    ...
    # landlock.py does `import platform` (module attribute read at call
    # time), so the platform.system patch above really is what its guard
    # sees; the CDLL patch in the When step covers the syscall path even if
    # that ever changes.
...
def pool_all_workers_fail(ctx):
    ...
    pool._started = True
    # Local pool (never installed as the singleton) — no reset needed.
