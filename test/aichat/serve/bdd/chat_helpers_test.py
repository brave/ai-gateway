...
def given_message_list(description, ctx):
    ...
    elif description == "no_messages":
        messages = []
    else:
        raise ValueError(f"unknown message list description: {description!r}")
...
def when_abandoned_tool_call(ctx, monkeypatch):
    ctx["empty_metric_before"] = (
        REGISTRY.get_sample_value(
            ...
            {"model": "test-model", "reason": "incomplete_tool_call"},
        )
        or 0.0
    )
    ctx["events"] = run_stream_pipeline(
        ctx, monkeypatch, chunks=[client_tool_call_chunk()]
    )
...
def when_content_chunk(content, ctx, monkeypatch):
    ...
    ctx["events"] = run_stream_pipeline(ctx, monkeypatch, chunks=[chunk])
...
def then_chunk_model(expected, ctx):
    ...
    # The echoed model must be the requested model AND the raw chunk model
    # (with spaces) must be gone from every chunk.
    assert models == [expected] * len(models) and models, models
    assert not any(" " in m for m in models), models
...
def then_streaming_research(name, ctx):
    ...
    # The handler must have received the completed deep_research tool call.
    calls = ctx["streaming_tool_kwargs"]["streaming_tool_calls"]
...
def when_mcp_event_and_result(ctx, monkeypatch):
    ...
    ctx["tool_result"] = "tool output"

    async def tool_events(*args, **kwargs):
        yield ('data: {"brave-chat.toolStart":true}\n\n', None)
        yield (
            None,
            ToolMessage(role="tool", tool_call_id="call_1", content=ctx["tool_result"]),
        )
