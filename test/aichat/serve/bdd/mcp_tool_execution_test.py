...
def then_error_chunk(ctx):
    # execute_tools_and_stream_events yields (sse_event, output_chunk)
    # tuples. The second slot is polymorphic by design: a dict chunk on
    # error paths (then_error_chunk) or a ToolMessage when the tool result
    # becomes a message (then_error_tool_message filters for ToolMessage).
...
def _default_backend_request(ctx):
    ...
    ctx["request"] = _FakeFollowupRequest()
    ctx["prompts_obj"] = None
    ctx["model_config"] = MagicMock()


class _FakeFollowupRequest:
    """Minimal request for handle_streaming_tool_calls follow-ups."""
    ...
    def __init__(self):
        # Per-instance list: the code under test may mutate request.messages,
        # and a class-level list would leak across scenarios.
        self.messages = [UserMessage(role="user", content="hello")]
    ...
    def model_copy(self, update=None):
        ...
        if update:
            data.update(update)
        # SimpleNamespace is safe here: handle_streaming_tool_calls only
        # reads attributes (model/stream/selected_language/messages) and
        # never calls pydantic methods on the copy.
...
def when_handle_streaming_calls(ctx, monkeypatch):
    ...
    monkeypatch.setattr(open_ai_api, "process_streaming_response", fake_process)
    ctx["chunks"] = _drain(
        mcp_tool_execution.handle_streaming_tool_calls(
            streaming_tool_calls=ctx["streaming_calls"],
            conversation_log_id="conv-1",
            actual_model="actual-model",
            mcp_tool_calls=ctx.get("mcp_calls", []),
            client_tool_calls=[],
            assistant_tool_calls_for_message=ctx.get("assistant_calls", []),
            request=ctx["request"],
            backend=ctx["backend"],
            model_config=ctx["model_config"],
            prompts_obj=ctx["prompts_obj"],
