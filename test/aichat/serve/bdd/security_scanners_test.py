...
from test.aichat.serve.bdd.helpers import completion_response as _completion
...
def settings_restore():
    ...
    except Exception as reload_error:  # teardown must not mask test failures
        logging.getLogger(__name__).warning("scanner reload failed: %s", reload_error)
...
def when_scan_task_raises(ctx, monkeypatch, caplog):
    ctx["_mp"] = monkeypatch
    ...
    with caplog.at_level(
        logging.WARNING, logger="aichat.services.alignment_check_service"
    ):
        _run_alignment_scan(
            ctx,
            "read the page",
    ...
    ctx["task_records"] = [r.message for r in caplog.records]


@when("the alignment scan backend returns an error dict")
def when_scan_backend_error(ctx, monkeypatch):
    ctx["_mp"] = monkeypatch
    _run_alignment_scan(ctx, "read the page", {"type": "error", "content": "boom"})
...
def then_task_failure_logged(ctx):
    assert any("Injection scan task failed" in m for m in ctx["task_records"])
...
def when_injection_backend_error(ctx, monkeypatch):
    ctx["_mp"] = monkeypatch
    _run_injection_scan(ctx, "page content", {"type": "error", "content": "boom"})
...
def when_alignment_disabled(ctx, settings_restore):
    mp = settings_restore
...
def when_injection_disabled(ctx, settings_restore):
    mp = settings_restore
