...
@then("a second emission carries no metadata")
def then_second(ctx):
...
@then(
    parsers.parse(
        'the truncated response starts with "..." {ellipsis} and keeps the tail'
    )
)
def then_truncated(ellipsis, ctx):
    truncated = ctx["truncated"]
    if ellipsis == "yes":
        assert truncated.startswith("...")
    else:
        assert not truncated.startswith("...")
    # Truncation always preserves the response tail.
...
def given_user_messages(description, ctx):
    ...
    elif description == "no_user_message":
        ctx["user_messages"] = [AssistantMessage(content="assistant only")]
    else:
        raise AssertionError(f"unknown user message description: {description!r}")
...
def given_history(description, ctx, monkeypatch):
    ...
    elif kind == "tool_results_without_an_assistant_call":
        # A tool message with no preceding assistant tool call still counts
        # as untrusted (no id→tool-name mapping can bypass it).
        ctx["history"] = [
            UserMessage(content="hi"),
            ToolMessage(role="tool", tool_call_id="call_1", content="fetched page"),
        ]
...
def when_launch_scan(ctx):
    async def run():
        # Honor the history Given; only fall back when none was staged.
        task = open_ai_adapter.maybe_perform_prompt_injection_scan(
            ctx.get("pi_history") or _untrusted_history(),
            ctx.get("pi_capability"),
        )
        if task is not None:
            task.cancel()
...
def then_scan_started(started, ctx):
    if started == "yes":
        assert ctx["scan_task"] is not None
    else:
...
def when_ns_injection(ctx):
    task_state = ctx.get("ns_task_state")

    async def run():
        if task_state == "completes":
            task = asyncio.create_task(
                _resolve(SimpleNamespace(probability=3, reasoning="ok"))
            )
            ...
            task = None
        try:
            return await open_ai_adapter.process_non_streaming_prompt_injection_scan(
                ctx["ns_payload"], task
            )
        finally:
    ...
    ctx["ns_injected"] = asyncio.run(run())
